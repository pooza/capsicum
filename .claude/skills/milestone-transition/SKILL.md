...
直近マイルストーンが**全プラットフォーム公開完了**した後、次のマイルストーンに着手するまでの間に毎回行う一連の整備手順。[sync-procedure](../sync-procedure/SKILL.md)（セッション開始時の同期）と [store-release-guide.md](../../../docs/store-release-guide.md)（ストアリリース）の間を埋める。

⚠⚠ **正本はこのファイル (#1114)。**[docs/milestone-transition.md](../../../docs/milestone-transition.md) はここへのポインタ。**手順を直すときはこちらだけを直す。**

## 実行タイミング

- 直近マイルストーンの残 Issue が 0、かつ全 5 プラットフォーム（Android / iOS / macOS / Windows / Linux）で公開完了を確認したとき。
- 判定は [sync-procedure](../sync-procedure/SKILL.md) の同期結果を流用してよい（リリースタグ・Play/ASC API・GitHub Release）。公開未完了なら着手準備は早いので、この手順には入らない。

## 手順

判断規約の正本は [CLAUDE.md の「マイルストーン運用」](../../../docs/CLAUDE.md#マイルストーン運用)。ここでは「移行時に何を・どの順で回すか」を定める。
...
#### 必須: capsicum 側 description の `## relay` 節（2026-08-22 義務化）

**capsicum のマイルストーン description には、例外なく `## relay` 節を置く。**触る分が無い回も「なし」と明示的に書く。節が無い状態＝規約違反として扱う（判断したのか忘れたのかが区別できないため）。

- **relay 側に同名枠がある回**: 枠へのリンク・件数・**Issue ごとの 1 行**（番号 / 要点 / 着手上の制約や順序）を書く。capsicum 側の Issue 一覧と同じ粒度にする。
- **無い回**: 次のとおり 1 行で書く。確認日を必ず付ける（stale を検出できるようにするため）。

  ```markdown
  ## relay

  なし（触る分が出た時点で同名枠を作る）。2026-08-22 確認。
  ```

⚠ **`なし` と書いたまま放置しない。** 2026-08-22 に v1.59 で、relay 側に同名枠（3 件 open）を作ったのに capsicum 側の節が「なし」のまま残っていた。その結果、**進捗報告のたびに relay が残件の数から落ちた**（pooza から 4〜5 回の指摘）。同名枠を作った / 枠へ Issue を足した回は、**同じ作業の中で capsicum 側 description も更新する**。同期のステップ 5（description 点検）でも実態と照合する。
...
- **出荷完了した版の `project_v1XX_progress` は削除する**。進捗メモリは**稼働中のマイルストーン 1 本だけ**に保つ（2026-07-17 方針化）。出荷済みの内容は GitHub Releases / Milestones（正本）と step 7 で退避した [archive/release-log.md](../../../docs/archive/release-log.md) に既にあり、メモリに残すと 4 重管理になってメモリ索引が肥大する。削除前に、GitHub からは読めない事実（資格情報の満了日・環境の癖・再発する罠）が埋もれていないか確認し、あれば step 7 と同じ要領で docs か `reference_*` メモリへ先に昇格させる。
