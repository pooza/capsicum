...
// 通常運転なので正常系に含める (#997)。
//
// `bgtask.degraded_shown` / `wns.degraded_skipped` は relay が本文を落とした
// 通知 (capsicum-relay#65) を汎用文面で出した / 起動中なので出さなかった、で
// いずれも想定どおりの結末。本文を落としたこと自体は relay 側が
// "Push oversized degraded" として warning で数えている。
bool IsBenignCode(const std::string& code) {
  return code == "bgtask.shown" || code == "bgtask.announcement_shown" ||
         ...
         code == "bgtask.degraded_shown" || code == "wns.degraded_skipped";
}
