...
class FlashRuntime {
  ...
  /// この Play が乱数を使うか (#935)。
  ///
  /// 使うなら「実行のたびに結果が変わる」「1.x 宣言の Play を force-run した
  /// ときは Web 版と出目が異なる」という注記を出す (#896 で条件が絞られた)。
  /// 関係の無い Play にまで出すと単なる雑音になるので絞る。
  ///
  /// 出目に効く評価器の乱数源は `Math:rnd`（毎回変わる）と `Math:gen_rng`
  /// （シード付き）の **2 つだけ**（`Util:uuid` も乱数だが識別子の生成なので
  /// 注記の対象にしない）。網羅は `test/flash_runtime_test.dart` が
  /// フォークの `lib/` を実際に読んで担保する (#960)。
  ///
