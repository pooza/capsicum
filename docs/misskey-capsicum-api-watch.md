...
プリセットサーバーの Flash を取得してパース・実行し、失敗を 3 分類する。分類がそのまま次のアクションに対応する:

| 分類 | 対応 |
| --- | --- |
| パース失敗 | AiScript の言語側が前進した → フォークの追従が必要 |
| ホスト供給変数の不足 | capsicum 側で変数の供給を足す |
| `Ui:` / `Mk:` の不足 | バインディング層に実装を足す |
| 実行時エラー | 評価器の非互換を個別に調査 |
