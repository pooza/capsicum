...
- **v1.6** — ユーザー要望の継続消化（翻訳・ハッシュタグフォロー・Misskey Play・言語選択・予約投稿タグ編集・絵文字パレットインポート・バグ修正 等）
- **v1.7** — 追加機能（通知統合一覧・実況タグ切替・Misskey ドライブ・メディアカタログ・アンケート作成・DM タイムライン 等）
- **v1.8** — UX 改善・Misskey 固有機能（公開範囲デフォルト・引用許可範囲指定・isCat・アンテナ・サーバー情報画面・検索強化 等）
