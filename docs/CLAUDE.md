...
最新リリース: **v1.37.0**（2026-06-16 タグ、**Apple Music ナウプレ（iOS / macOS）**を主役とする大更新単独配置、build 118、PR [#706](https://github.com/pooza/capsicum/pull/706)）。公開状況: **Android 製品版公開済み**（versionCode=118、production completed を Play API で確認）/ **iOS App Store 審査提出済み**（build 118、submitted for review）/ **macOS Mac App Store は pooza 手動 MAS 提出待ち**（TestFlight 内部ベータ配信済み。temporary-exception 審査 Notes は store-release-guide §4.3）/ **Linux AppImage + Windows MSIX** は [GitHub Release v1.37.0](https://github.com/pooza/capsicum/releases/tag/v1.37.0) draft に添付・ノート設定済み（publish は pooza）/ **Windows Microsoft Store は pooza 手動 publish**。主役は [#668](https://github.com/pooza/capsicum/issues/668) Apple Music ナウプレ pull（iOS=MPMusicPlayerController / macOS=ミュージック.app scripting）。macOS は App Sandbox 下で modern の `automation.apple-events` 単独だとミュージックが addressable と認識されず procNotFound(-600)。`com.apple.security.temporary-exception.apple-events`=["com.apple.Music"] を併用して解決（build 117 実機確認、内部ベータ 109-117 の Sentry で確定）。あわせて [#669](https://github.com/pooza/capsicum/issues/669) enrich（URL なし源に共有 URL 補完）/ [#670](https://github.com/pooza/capsicum/issues/670) タグ末尾規律 / [#703](https://github.com/pooza/capsicum/issues/703)「削除して再編集」で CW/添付/閲覧注意の引き継ぎ + リモートメンション復元 / [#704](https://github.com/pooza/capsicum/issues/704) account secret 喪失の観測性 / [#709](https://github.com/pooza/capsicum/issues/709) 挿入後キャレット / [#695](https://github.com/pooza/capsicum/issues/695) iOS/macOS 英語ローカリゼーション削除 / [#696](https://github.com/pooza/capsicum/issues/696)-[#699](https://github.com/pooza/capsicum/issues/699) v1.36 レビュー followup / [#721](https://github.com/pooza/capsicum/issues/721) Mastodon 4.6 互換確認（受動・破壊的変更なし）を消化。リリース前レビュー（5観点 + 大更新につき自己点検）は赤0（API契約観点の getMaxProfileFields 赤候補は誤判定＝06bebad は存在しない v1 フィールド `?? 4` で常に 4 を返す誤った上限の撤去＝改善と確認）。黄/緑は [63cf26b](https://github.com/pooza/capsicum/commit/63cf26b) でインライン消化（profile 422 の Sentry を構造化シグナル化＝生本文を載せない / getMaxProfileFields コメント事実訂正＝Mastodon は上限を API 露出しない / enrich URL を http/https 限定 / macOS `__scriptItems` 死にコード削除）。構造的黄は [#724](https://github.com/pooza/capsicum/issues/724)（supporter backfill 失敗時の再試行抑止）で v1.38。secrets は iOS App.framework / Android arm64 libapp.so / macOS App.framework で実バイナリ確認、macOS 再署名 `.pkg` は aps-environment=production・temporary-exception 維持・Apple Distribution 署名を確認。pubspec 1.37.0+118。

**v1.36.1**（2026-06-12 タグ、PR [#702](https://github.com/pooza/capsicum/pull/702) / release/1.36.1、build 108。v1.36.0 リリース後レビューのホットフィックス）。公開状況: **全プラットフォームで 1.36.1 公開済み** — **Android 製品版公開済み**（versionCode=108、production completed を Play API で確認）/ **iOS App Store 公開済み**（2026-06-13 公開、lookup version=1.36.1）/ **macOS Mac App Store 公開済み**（2026-06-13 公開、lookup version=1.36.1）/ **Linux AppImage + Windows MSIX 自己署名直配**は [GitHub Release v1.36.1](https://github.com/pooza/capsicum/releases/tag/v1.36.1) で publish 済み / **Windows Microsoft Store は pooza 手動 publish（順次反映）**。消化: [#700](https://github.com/pooza/capsicum/issues/700) push 復号失敗ログの scrub 漏れ / [#701](https://github.com/pooza/capsicum/issues/701) 連続ログイン時のサポーター同期 race。Codex #702 P2（backfill 経路の race）は #701 commit で同梱対処・返信 + 👍 済み。pubspec 1.36.1+108。

**v1.36.0**（2026-06-12 タグ、**Misskey chat / Pages 続編 + 投げ銭の macOS 横展開・サーバー側保持**の集約枠・大更新なし。v1.36.1 ホットフィックスで上書き）。公開状況: **Android 製品版公開済み**（versionCode=107 を Play API で確認）/ **iOS App Store 審査提出済み**（build 107、WAITING_FOR_REVIEW を ASC API で確認）/ **macOS Mac App Store 公開済み**（build 107、2026-06-13 公開。pooza 手動提出 → 審査通過。初回アップロードは Transporter の 500 応答で失敗に見えたが実は受理済み＝再試行の Redundant Binary Upload で確認）/ **Windows Microsoft Store は pooza 手動 publish 待ち** / **Linux AppImage + Windows MSIX** は draft Release に添付・ノート設定済み（publish は pooza）。消化: [#612](https://github.com/pooza/capsicum/issues/612) メッセージのリアクション / [#613](https://github.com/pooza/capsicum/issues/613) メッセージのファイル添付 / [#615](https://github.com/pooza/capsicum/issues/615)・[#617](https://github.com/pooza/capsicum/issues/617) Pages の like・人気セクション（read/write:page-likes スコープ追加、旧トークンは再ログイン誘導）/ [#598](https://github.com/pooza/capsicum/issues/598) macOS 投げ銭 IAP / [#596](https://github.com/pooza/capsicum/issues/596) サポーター状態サーバー側保持（relay#18）/ [#673](https://github.com/pooza/capsicum/issues/673)・[#674](https://github.com/pooza/capsicum/issues/674) macOS 通知後始末（NSE は OS レベルで機能せず断念 → DeliveredPushCleaner 方式）/ [#477](https://github.com/pooza/capsicum/issues/477) お知らせ push を mobile 限定 / [#646](https://github.com/pooza/capsicum/issues/646) モバイルのギャラリー保存（gal）/ [#693](https://github.com/pooza/capsicum/issues/693) ドライブ移動モーダル現在地ベース + 自動スクロール / [#689](https://github.com/pooza/capsicum/issues/689) 挿入ピッカー高さ / [#658](https://github.com/pooza/capsicum/issues/658) ログイン画面サーバー画像 / [#683](https://github.com/pooza/capsicum/issues/683)・[#684](https://github.com/pooza/capsicum/issues/684)・[#665](https://github.com/pooza/capsicum/issues/665) 修正。リリース前レビュー（5観点）は赤0・黄3・緑2、全件 followup 起票で v1.37 へ（[#696](https://github.com/pooza/capsicum/issues/696) native ログのユーザー識別子 / [#697](https://github.com/pooza/capsicum/issues/697) supporter sync breadcrumb / [#698](https://github.com/pooza/capsicum/issues/698) catch 内 ref.read、検証済みベータ +106 からの挙動変更を避ける判断）。Codex P2 は 2 件: サポーター新規アカウントのバックフィル漏れ → [#699](https://github.com/pooza/capsicum/issues/699)（v1.37）、macOS IAP entitlement 指摘は空振り判定（当該 entitlement は存在せず + TestFlight 実機検証済み）。**Windows CI が runner image 更新（2026-06-12）で 2 段破損**: CMake 4.x が firebase_cpp_sdk_windows の古い cmake_minimum_required を拒否（→ workflow に `CMAKE_POLICY_VERSION_MINIMUM=3.5`）、MSVC 14.51 (VS 18) が gal の `<experimental/coroutine>` 非推奨 assert をエラー化（→ windows/CMakeLists.txt に `_SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS`）。修正はタグ後の develop 積みのため v1.36.0 タグの Windows run は failure のまま、MSIX は develop からの workflow_dispatch 成果物を draft Release に手動添付。secrets は iOS（App.framework）・Android（arm64 libapp.so）・macOS（xcarchive App.framework）とも SENTRY_DSN 焼き込みを実バイナリで確認。pubspec 1.36.0+107。リリース PR [#682](https://github.com/pooza/capsicum/pull/682)。

**v1.35.0**（2026-06-09 タグ、**投稿サジェスト（劇中ワード）**を主役とする大更新単独配置、**全プラットフォーム公開済み**）。公開状況: **Android 製品版公開済み** / **iOS App Store 公開済み（2026-06-09 審査通過）** / **macOS Mac App Store 公開済み**（build 103、READY_FOR_SALE を 2026-06-12 確認）/ **Windows Microsoft Store 公開済み**（build 103、pooza 手動）+ 自己署名 MSIX 直配 / **Linux AppImage + Windows MSIX** は [GitHub Release v1.35.0](https://github.com/pooza/capsicum/releases/tag/v1.35.0) で publish 済み。主役は [#614](https://github.com/pooza/capsicum/issues/614) 投稿フォームの劇中ワードサジェスト（読み＝ひらがなで [mulukhiya #4397](https://github.com/pooza/mulukhiya-toot-proxy/issues/4397) `GET /word/suggest` を引いて挿入。辞書を用意したモロヘイヤ導入サーバーのみ、`config.features.word_suggest` で出し分け。設計 [compose-suggest-design.md](compose-suggest-design.md)）。絵文字（カスタム / Unicode）と劇中ワードを同一シートから開く共有ランチャ [insert_picker_sheet.dart](../packages/capsicum/lib/src/ui/widget/insert_picker_sheet.dart) に compose / 簡易投稿バーを集約。簡易投稿バーの UX 調整（絵文字履歴トグルを `Icons.history` 化してフルピッカーと識別 / アクションアイコンを `IconButtonTheme` で 36px コンパクト枠に詰め小型端末の TextField 幅確保 / 挿入ピッカー選択後に入力欄へフォーカス復帰）と AppBar アイコンの同コンパクト化も同梱。リリース前レビュー（5観点）は赤0、黄はコメント整備のみインライン消化（[d242d34](https://github.com/pooza/capsicum/commit/d242d34)）、大更新の2回目差分レビューは1回目修正がコメント・整形のみ（ロジック不変）のため format/analyze 再確認で代替。ビルド番号は並行ビルド（複数端末）で交錯し、当初 Android は朝の**マージン調整前 102** が誤って製品版に昇格していた（`fastlane internal` を `| tail` にパイプして versionCode 衝突の exit code を取りこぼし、続く `fastlane release` が既存 102 を昇格してしまった）。pooza の実機確認で発覚し、**Android を `--build-number=103` で再ビルド→再昇格**して是正（製品版 versionCode=103 を Play API で確認）。最終ビルド番号は **iOS/macOS/Windows/Android とも 103**。secrets は iOS 103（App.framework）・Android 103（全 arch libapp.so）とも RELAY_SECRET/SENTRY_DSN 焼き込みを実バイナリで確認（[+50 の空展開罠](store-release-guide.md)対策）。pubspec は 1.35.0+102（Android のみ build-number 上書きで 103）。リリース PR [#679](https://github.com/pooza/capsicum/pull/679)。

**v1.34.0**（2026-06-07 タグ、**デスクトップ通知本配線**を主役とする大更新単独配置）。公開状況: **Android 製品版公開済み** / **iOS App Store 審査提出済み**（build 100、WAITING_FOR_REVIEW）/ **macOS Mac App Store 審査提出済み**（build 101、WAITING_FOR_REVIEW。TestFlight 配信後に pooza が手動提出）/ **Windows Microsoft Store 公開済み**（2026-06-07、pooza 手動）+ 自己署名 MSIX 直配 / **Linux AppImage** は [GitHub Release v1.34.0](https://github.com/pooza/capsicum/releases/tag/v1.34.0) で publish 済み。主役は [#569](https://github.com/pooza/capsicum/issues/569) デスクトップ 3 OS で WebSocket streaming（Mastodon user / Misskey main）→ OS ローカル通知 / [#468](https://github.com/pooza/capsicum/issues/468) macOS APNs 本配線（capsicum-relay 連携。FlutterAppDelegate(macOS) は didFailToRegister/didRegister が runtime 未実装のため super を呼ばず override 内で完結する macOS 固有対応。設計 [desktop-notification-design.md](desktop-notification-design.md)）。あわせて [#675](https://github.com/pooza/capsicum/issues/675) 全ログインアカウント受信 / [#592](https://github.com/pooza/capsicum/issues/592) Annict review 投稿 / [#611](https://github.com/pooza/capsicum/issues/611) 未連携時の連携誘導 / [#576](https://github.com/pooza/capsicum/issues/576) 画像への文字・絵文字オーバーレイ / [#660](https://github.com/pooza/capsicum/issues/660) 画像プレビュー / [#662](https://github.com/pooza/capsicum/issues/662) トリミング回転 / [#593](https://github.com/pooza/capsicum/issues/593) エピソードブラウザのハッシュタグ操作集約 を消化。リリース前レビュー（5観点 + 大更新につき2回目差分）は赤2（Misskey 通知 streaming の onDone backoff リセットによるタイト再接続ループ / 公開 docs の ASC Key ID・issuer 露出）・黄4（通知 streaming の観測性コールバックを interface→adapter→dispatcher→Sentry まで配線 / macOS APNs onDeviceTokenError 計装 / dispatcher の OS 通知 show 計装 / overlay 画面の mounted ガード）をインライン消化（[aa44d97](https://github.com/pooza/capsicum/commit/aa44d97)）、2回目差分レビューはクリーン（赤0）。構造的指摘は followup [#676](https://github.com/pooza/capsicum/issues/676)（通知 streaming 再接続機構の共有基底化、#627 同型）/ [#677](https://github.com/pooza/capsicum/issues/677)（Annict review の feature-gate）。Codex P2（macOS 起動中の二重通知）は既知 [#674](https://github.com/pooza/capsicum/issues/674) に紐付け返信 + 👍。relay は #468 で device_type='macos' 許容 + announcement_subscriptions の FK 破損修復（flauros デプロイ済み）。#674 要因2（relay の同一アカウント macOS token 重複）は調査の結果ベータ多ビルド由来で**製品版は無実害**と判明し、flauros の重複行は最新1行に集約済み（13→5）。既知の不具合: [#674](https://github.com/pooza/capsicum/issues/674) macOS 起動中の二重通知（#569×#468 の横断 dedup 未実装、設計上 Phase B）/ [#673](https://github.com/pooza/capsicum/issues/673) macOS APNs 文面の NSE 復号未実装。pubspec 1.34.0+100（macOS のみ +101）。リリース PR [#671](https://github.com/pooza/capsicum/pull/671)。

**v1.33.0**（2026-06-06 タグ、**desktop ネイティブ NowPlaying** を主役とする大更新単独配置）。公開状況: **Android 製品版公開済み** / **iOS App Store 公開済み（2026-06-06 審査通過）** / **macOS Mac App Store 公開済み（2026-06-07 審査通過）**（審査停滞していた v1.32.0 をキャンセルして v1.33 を提出・置換し停滞解消）/ **Windows Microsoft Store 公開済み** + 自己署名 MSIX 直配 / **Linux AppImage** は [GitHub Release v1.33.0](https://github.com/pooza/capsicum/releases/tag/v1.33.0) で publish 済み。主役は [#466](https://github.com/pooza/capsicum/issues/466) NowPlayingProvider 抽象 + Linux MPRIS / [#484](https://github.com/pooza/capsicum/issues/484) Windows SMTC（OS のメディア情報を pull して compose に挿入。整形はクライアント側に確定、設計 [nowplaying-design.md](nowplaying-design.md)）。あわせて [#666](https://github.com/pooza/capsicum/issues/666) Misskey ログイン直後のチャンネルタブ表示を消化。責務分担見直しで Apple Music（[#668](https://github.com/pooza/capsicum/issues/668)、v1.37）/ Spotify（[#570](https://github.com/pooza/capsicum/issues/570)、v1.38）/ enrich 配線（[#669](https://github.com/pooza/capsicum/issues/669)、v1.37）は別マイルストーンへ分離。リリース前 5 観点レビューは赤0、黄4件（連打ガード / 観測性 breadcrumb / 入力正規化 / Windows SMTC の UI スレッド wait_for タイムアウト）をインライン消化（[7c9bd60](https://github.com/pooza/capsicum/commit/7c9bd60)）、設計依存の黄1件（共有動線のタグ統一）は followup [#670](https://github.com/pooza/capsicum/issues/670)。Windows SMTC ネイティブ変更は windows-release.yml の MSIX artifact で実機検証済み。pubspec 1.33.0+96。リリース PR [#667](https://github.com/pooza/capsicum/pull/667)。
