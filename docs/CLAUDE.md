...
- **v1.12** — Misskey 固有機能 + ユーザー要望（絵文字デッキ・実績・ページ・背景画像・ダークモード詳細カスタマイズ・削除してタグづけの mulukhiya API 移行・プリフェッチ改善）
- **v1.13** — DM・メッセージ + バグ修正（DM タイムライン・Misskey メッセージ・Share Extension パス修正・確認画面条件・実況タグフィルタ・復号エラー対応）
- **v1.14** — 通知統合 + 下書き + ユーザーフィードバック対応（通知・お知らせ統合一覧・サーバーサイド下書き・通知欄アクション・ナウプレ削除・Android ログイン調査）
- **v1.15** — 連携（ポイピク連携・メディア API v2）
