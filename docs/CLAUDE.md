...
最新リリース: **v1.54.0**（2026-08-07 タグ、build 167、pubspec 1.54.0+167、リリース PR [#929](https://github.com/pooza/capsicum/pull/929)、merge `b10e29a8`）。**大更新なし — 1.x アグリゲーション枠**。公開状況（2026-08-08 実測）: **Android 製品版公開済み**（2026-08-07 に production track の versionCode 167 を Play API で実測確認）/ **iOS App Store 公開済み**（lookup で 1.54.0・`currentVersionReleaseDate` 2026-08-08T00:08Z）/ **macOS Mac App Store 公開済み**（同上）/ **Windows Microsoft Store 公開済み**（pooza 手動 publish。displaycatalog でパッケージ版 1.54.167.0 を確認）/ **Linux AppImage + Windows MSIX** は [GitHub Release v1.54.0](https://github.com/pooza/capsicum/releases/tag/v1.54.0) で publish 済み（Latest）。**全 5 プラットフォーム公開完了**。マイルストーン [#68](https://github.com/pooza/capsicum/milestone/68) は close 済み。消化（17 件）: [#883](https://github.com/pooza/capsicum/issues/883) 添付画像へのカスタム絵文字スタンプ重ね合わせ / [#900](https://github.com/pooza/capsicum/issues/900) アダプターの Dio に既定タイムアウト / [#907](https://github.com/pooza/capsicum/issues/907) リアクション絵文字ピッカーの高さ調整 / [#912](https://github.com/pooza/capsicum/issues/912) デスクトップメニューの画面連動を残り画面へ拡大 / [#914](https://github.com/pooza/capsicum/issues/914) 起動キャッシュの残課題（保存先の機微度ほか）/ [#916](https://github.com/pooza/capsicum/issues/916) 再試行ボタンがスピナーに戻らない / [#917](https://github.com/pooza/capsicum/issues/917) オフライン起動でサーバー選択画面へ飛ぶ / [#920](https://github.com/pooza/capsicum/issues/920) ホバーの「誰がやったか」が別投稿の結果を表示 / [#921](https://github.com/pooza/capsicum/issues/921) 起動キャッシュ先出し中の操作が捨てられる / [#922](https://github.com/pooza/capsicum/issues/922) Windows / Linux のメニューバー編集項目 / [#930](https://github.com/pooza/capsicum/issues/930) ログイン成功後の null check クラッシュ / [#932](https://github.com/pooza/capsicum/issues/932) プッシュ登録に安定した device-id / [#933](https://github.com/pooza/capsicum/issues/933) Windows の WebSocket / WNS 二重通知 / [#934](https://github.com/pooza/capsicum/issues/934) Play のバージョン警告を事実ベースへ / [#935](https://github.com/pooza/capsicum/issues/935) Play の乱数注記を独立掲出 / [#936](https://github.com/pooza/capsicum/issues/936) 「同じ通知が繰り返し届く」報告の切り分け / [#937](https://github.com/pooza/capsicum/issues/937) 再起動をまたぐ device token 変化で購読が孤児化。

**リリース前レビューは 1 巡で打ち切った**（🔴 4 件・🟡 13 件・🟢 多数）。v1.53 の 6 巡は異常時の対応で、通常は 1 巡が基準（🔴 が巡回をまたいで続く場合のみ繰り返す）。**🔴 のうち 3 件は、このマイルストーン自身が入れた回帰**だった: #883 の書き出し経路の use-after-free（`_render` が codec デコードを await した後に `_items` を走査するため、await 中に pop されると dispose 済みの `ui.Image` を描画。`PopScope` が無く `_rendering` は AppBar のボタンしか止めていなかった）と、#900 のタイムアウト値 2 つ。**dio 5.11 の実装を読むと `receiveTimeout` は `io_adapter` の「ヘッダー到達までの絶対上限」と `response_stream_handler` の「body チャンク間隔」の 2 箇所で使い分けられており、`BaseOptions` の doc が書いているのは後者だけ**。前者はサーバーが計算している時間の上限になるので、30 秒だと Mastodon の翻訳（サーバー側 35 秒）や `search(resolve: true)` を client が先に切る。`sendTimeout` も間隔ではなく送信ボディ全体の壁時計上限で、5 分だと大きな動画を細い回線で送れない。60 秒 / 20 分へ引き上げ、doc を実装で確認した意味へ書き直した（根拠となる実測値も doc に残した）。経緯の正本は PR [#929](https://github.com/pooza/capsicum/pull/929) 本文。**overflow 11 本はすべてマイルストーン付きで起票**（v1.55: [#950](https://github.com/pooza/capsicum/issues/950)-[#955](https://github.com/pooza/capsicum/issues/955) / v1.56: [#956](https://github.com/pooza/capsicum/issues/956)-[#959](https://github.com/pooza/capsicum/issues/959) / v1.57: [#960](https://github.com/pooza/capsicum/issues/960)）。うち [#952](https://github.com/pooza/capsicum/issues/952) は **capsicum-relay#15 のデプロイ前に決着が要る**（device_id を保存する SharedPreferences が OS バックアップで別端末へ複製されるため、upsert を入れると復元した端末と元の端末のどちらかに push が届かなくなる）。

検証は内部ベータ `1.54.0 (167)` を iOS / macOS / Android へ配信して実機確認。**3 プラットフォームとも同一の build 167 を提出**（`skip_binary_upload: true` 運用のため、実機確認したバイナリをそのまま審査へ回せる）。ASC が build 167 に対し `90068 MinimumOSVersion too low`（14.0 → 2027 年春までに 15.0 以上が必須）を警告したが提出はブロックされず、[#961](https://github.com/pooza/capsicum/issues/961) として v1.57 へ。
