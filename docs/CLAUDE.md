...
最新リリース: **v1.65.0**（2026-09-17 タグ、build 184、pubspec 1.65.0+184、リリース PR [#1118](https://github.com/pooza/capsicum/pull/1118)、merge `e5f63574`）。**大更新なし — 読む側の実害を潰す枠**（引っ越し / 凍結の印・編集済みの印・本文カウンタ）+ ログインまわりの実機検証由来の不具合。**公開状況（2026-09-17 実測）**: Android は production track の versionCode 184 が `completed`（Play API）/ Linux AppImage と Windows MSIX は [GitHub Release v1.65.0](https://github.com/pooza/capsicum/releases/tag/v1.65.0)（Latest）/ **iOS は `WAITING_FOR_REVIEW`・macOS は `IN_REVIEW`**（ASC API）/ **Windows は Partner Center へ提出済みで認定待ち**（displaycatalog はまだ `1.64.183.0`）。マイルストーン [#79](https://github.com/pooza/capsicum/milestone/79)。消化（13 件）: [#1055](https://github.com/pooza/capsicum/issues/1055) 引っ越し済み・凍結・サイレンス・削除済みの印 / [#1054](https://github.com/pooza/capsicum/issues/1054) 編集済み投稿の印 / [#1034](https://github.com/pooza/capsicum/issues/1034) 本文カウンタを Mastodon の数え方へ / [#1119](https://github.com/pooza/capsicum/issues/1119) バックアップにアカウント別設定を含める / [#1110](https://github.com/pooza/capsicum/issues/1110) 足し直しで一覧に 2 件並ぶ / [#1109](https://github.com/pooza/capsicum/issues/1109) force_login の条件付け（⚠ **#1143 で無条件へ戻した**）/ [#1112](https://github.com/pooza/capsicum/issues/1112) ログイン試行の二重起動 / [#1111](https://github.com/pooza/capsicum/issues/1111) 3 分超過の案内 / [#1136](https://github.com/pooza/capsicum/issues/1136) secure storage の関所の層抜け / [#1117](https://github.com/pooza/capsicum/issues/1117) v1.64 レビューの送り分 / [#1114](https://github.com/pooza/capsicum/issues/1114) 手順書をスキルへ / [#1142](https://github.com/pooza/capsicum/issues/1142) v2.0 開発中の 1.x リリース手順の明文化 / [#1143](https://github.com/pooza/capsicum/issues/1143) リリース前レビューでリリース前に直すもの。送り分は [#1144](https://github.com/pooza/capsicum/issues/1144)（v1.66）。**relay v1.65 は残 0 で完了**（[relay#54](https://github.com/pooza/capsicum-relay/issues/54) WNS 接続の再利用 / [relay#64](https://github.com/pooza/capsicum-relay/issues/64) stale 再送がもう 1 本の stale を借りる）。⚠ **relay の open 8 件はすべて枠外**（#55 は #597 と同じ回 / #56 は on-hold / 有償リレー 6 件は v2.0）。
