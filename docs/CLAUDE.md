...
GitHub Issues のマイルストーン（v1.0 / v1.1 / v1.2 / v1.3 / v1.4 / v1.5 / v1.6 / v1.7 / v1.8 / v1.9 / v1.10 / v1.11 / v1.12）が正本。個別 Issue の一覧・ステータスはここに複写しない。
...
- **v1.10** — メディア周り + モロヘイヤ連携 + UX + サーバー情報改善（メディアカタログ・メディア API v2・ALT 編集・サムネイルサイズ・アンテナ・ナウプレ削除・投稿プレビュー・DM タイムライン・サーバー情報画面の微修正）
- **v1.11** — UX 拡張 + モロヘイヤ連携（NowPlaying・通知統合一覧・ドライブ管理・ポイピク連携・ステータスページリンク・URL コピー・タイムラインタブ切替）
- **v1.12** — Misskey 固有機能の拡充（isCat・実績・ページ・下書き・メッセージ）
