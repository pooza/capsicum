...
GitHub Issues のマイルストーン（v1.0 / v1.1 / v1.2 / v1.3 / v1.4 / v1.5 / v1.6 / v1.7 / v1.8 / v1.9 / v1.10 / v1.11）が正本。個別 Issue の一覧・ステータスはここに複写しない。
...
- **v1.9** — 設定画面整理 + 表示カスタマイズ + モロヘイヤ連携（設定画面構造整理・notestock 検索修正/改善・絶対時間表示・画像/OGP ぼかし・投稿前確認・リスト/タグTL 表示管理・予約投稿タグ編集・モロヘイヤ再検出）
- **v1.10** — メディア周り + モロヘイヤ連携 + UX（メディアカタログ・メディア API v2・ALT 編集・サムネイルサイズ・アンテナ・ナウプレ削除・投稿プレビュー・DM タイムライン）
- **v1.11** — 大型機能 + Misskey 固有（NowPlaying・通知統合一覧・ドライブ管理・下書き・ページ・実績・ポイピク連携）
