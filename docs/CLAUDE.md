...
2. **第2段階: バックグラウンド/通知モデルの再設計（v1.21）** — デスクトップにはバックグラウンド更新の概念がないため、通知ポーリングを抽象化して差し替え可能にする。v1.18 のプッシュ通知リレー完了後、workmanager 依存が下がったタイミングで整理する。workmanager / flutter_local_notifications のデスクトップ対応差分を吸収する層が要る
