...
最新リリース: **v1.41.1**（2026-06-28 タグ、build 137、pubspec 1.41.1+137、リリース PR [#783](https://github.com/pooza/capsicum/pull/783)）。**v1.41.0 後のホットフィックス（大更新なし）— ストリーミングの投稿取りこぼし修正**。公開状況: Android 製品版昇格済み / iOS App Store 審査提出済み（build 137 IN_REVIEW・routine 完走扱い）/ macOS Mac App Store・Windows Microsoft Store とも pooza が提出済み / Windows MSIX + Linux AppImage は [GitHub Release v1.41.1](https://github.com/pooza/capsicum/releases/tag/v1.41.1) を draft 作成済み（publish は pooza）。消化: [#781](https://github.com/pooza/capsicum/issues/781) streaming が未接続の窓（初回 REST→WS live・各再接続）に流れた投稿を取りこぼす問題を since_id ギャップ補完で解消（WS が live になるたび既知の最新 id を起点に REST 差分を取得し id 降順でマージ・Mastodon/Misskey 共通の `TimelineNotifier`）＋ Misskey streaming の `onDone` 再接続バックオフ・リセット不整合を Mastodon 側と統一。Sentry で `reconnect_exhausted`（CAPSICUM-25・4 ユーザー 42 回）を裏取り、変換失敗系（`timeline.stream.parse`/`Post conversion failed`）0 件で機構を「接続フラップのギャップ」と確定。ダイスキー実況で WebUI 横並び検証し取りこぼし解消を確認。Codex（#783）P2×2（catch-up の文脈ガード欠如・空 TL シード）を bbc14c6 でインライン消化。後続: 接続インジケータの正直・厳密化（再接続カウント等）は [#782](https://github.com/pooza/capsicum/issues/782)（v1.42）。pubspec 1.41.1+137。
