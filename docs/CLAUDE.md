...
⚠ **1.x と 2.x は並走系列で、境界は「時期」ではなく「土台を組み替えるか」。**`v2.0` は [#720](https://github.com/pooza/capsicum/issues/720)（デッキ UI）単独の枠で、同じ大更新でも [#597](https://github.com/pooza/capsicum/issues/597) / [#884](https://github.com/pooza/capsicum/issues/884) は土台を変えないので 1.x に置いている。**規模と系列は独立の軸**（規模は「独立枠に単独配置するか」を決めるだけ）。⚠ **2026-08-17〜 の「外部要因の発生ベース」運用は v1.65 をもって終了**した。
