...
最新リリース: **v1.57.0**（2026-08-16 タグ、build 170、pubspec 1.57.0+170、リリース PR [#977](https://github.com/pooza/capsicum/pull/977)、merge `d4f27091`）。**大更新なし — 1.x アグリゲーション枠**。公開状況（2026-08-17 実測・ASC API 個別確認）: **Android 製品版公開済み**（production track の versionCode 170 が `completed`）/ **iOS App Store 公開済み**（1.57.0 `READY_FOR_SALE`）/ **macOS Mac App Store 公開済み**（1.57.0 `READY_FOR_SALE`）/ **Windows Microsoft Store 公開済み**（pooza 手動 publish。displaycatalog で `9AFBB08E.capsicum_1.57.170.0_x64` を確認）/ **Linux AppImage + Windows MSIX** は [GitHub Release v1.57.0](https://github.com/pooza/capsicum/releases/tag/v1.57.0) で publish 済み（Latest）。**全 5 プラットフォーム公開完了**。マイルストーン [#71](https://github.com/pooza/capsicum/milestone/71) は close 済み。消化（15 件）: [#918](https://github.com/pooza/capsicum/issues/918) Misskey 実績解除を実績名つき通知に / [#919](https://github.com/pooza/capsicum/issues/919) デスクトップでもアプリ終了中にお知らせ通知（macOS 分）/ [#923](https://github.com/pooza/capsicum/issues/923) Misskey チャンネルリノート拒否の専用文言 / [#939](https://github.com/pooza/capsicum/issues/939) ドライブ画面のデスクトップメニュー / [#941](https://github.com/pooza/capsicum/issues/941) 投稿フォームの添付メディア操作をメニューから / [#943](https://github.com/pooza/capsicum/issues/943) 投稿アクションメニューの外部起動（`PostActionRunner` へ 1 本化）/ [#946](https://github.com/pooza/capsicum/issues/946) 画像オーバーレイのレイヤ回転 / [#948](https://github.com/pooza/capsicum/issues/948) debug ビルドのリレー向け先をステージングへ / [#960](https://github.com/pooza/capsicum/issues/960) v1.54 レビュー緑まとめ 17 項目 / [#961](https://github.com/pooza/capsicum/issues/961) iOS 最低要件 14.0 → 15.0 / [#963](https://github.com/pooza/capsicum/issues/963) サーバー下書きを投稿フォームから選択 / [#971](https://github.com/pooza/capsicum/issues/971) メニューバーの投稿メニュー / [#974](https://github.com/pooza/capsicum/issues/974) オフライン保持の一過性 secret 失敗が再試行ループに載らない / [#978](https://github.com/pooza/capsicum/issues/978) Windows: 無暗号化のお知らせ push を bg task で表示（実機ゲート）/ [#979](https://github.com/pooza/capsicum/issues/979) お知らせ購読解除の 404 を冪等に成功扱い。**relay v1.57**（[Milestone #8](https://github.com/pooza/capsicum-relay/milestone/8)・4 件、close 済み）: relay#2 可視性強化（構造化ログ / `/metrics`）/ relay#34 route を 5 クラスへ分割 + request テスト土台 / relay#36 お知らせ配送先を macOS（Phase 1）・Windows（Phase 2）へ拡張 / relay#37 `/health` に稼働 revision。

**リリース前レビューは 1 巡**（🔴 2・🟡 3 を出荷前に消化、`05a86f39`）。🔴 はいずれもこの版自身の回帰で、**「新機能が導線ごと不通」と「隣の機能を巻き添えで壊す」**の 2 型（#918 の実績通知タップが `extra` 無しで `state.extra!` の route に飛ぶ／#963 の `_clearDraft()` に自動保存ガードが無く無関係のローカル下書きを消す）。⚠ **#918 は実績を狙って解除できず動作確認できない**ため、`extra` 必須 route への遷移 43 件を機械的に洗って修正前の版でこの 1 件だけが挙がることを実測し担保した。送り先は [#982](https://github.com/pooza/capsicum/issues/982)（緑まとめ 18 項目）/ [#983](https://github.com/pooza/capsicum/issues/983) / relay [#45](https://github.com/pooza/capsicum-relay/issues/45)（すべて v1.58）。**Codex P2（ドライブの「上の階層へ」が複数選択中も動く）は [#984](https://github.com/pooza/capsicum/issues/984) として v1.58 へ送った** — 指摘時点で Apple 提出済み・Android 公開済みで、直すと同じ 1.57.0 でプラットフォーム間のバイナリが食い違うため（P2 に build 再提出は不釣り合いと判断）。実お知らせでの macOS / Windows 到達確認は任意に起こせないためリリース前ゲートから外し、[#981](https://github.com/pooza/capsicum/issues/981)（v1.58・検証のみで実装を伴わない枠）として出荷後に拾う。
