...
最新リリース: **v1.61.0**（2026-08-27 タグ、build 174、pubspec 1.61.0+174、リリース PR [#1031](https://github.com/pooza/capsicum/pull/1031)、merge `422f441d`）。**大更新なし — 1.x の集約枠・前進スロット**（v1.60 と同型）。公開状況（2026-08-27 実測）: Android 製品版（production track の versionCode 174 が `completed`）/ iOS `WAITING_FOR_REVIEW` / macOS `WAITING_FOR_REVIEW`（ASC API で実測）/ **Linux AppImage + Windows MSIX** は [GitHub Release v1.61.0](https://github.com/pooza/capsicum/releases/tag/v1.61.0) で publish 済み（Latest）/ **Windows は Partner Center からの手動 publish 待ち**（公開カタログはまだ `1.60.173.0` を返す）。マイルストーン [#75](https://github.com/pooza/capsicum/milestone/75)。消化（7 件）: [#1024](https://github.com/pooza/capsicum/issues/1024) オフラインアカウント削除で通知ラベルキャッシュが残る / [#1025](https://github.com/pooza/capsicum/issues/1025) 深くネストした YAML で UI が数十秒固まる / [#1026](https://github.com/pooza/capsicum/issues/1026) 投稿フォーム設定の flush がウィンドウを閉じる経路を覆っていない / [#1027](https://github.com/pooza/capsicum/issues/1027) v1.60 レビュー緑まとめ / [#1030](https://github.com/pooza/capsicum/issues/1030) Play を指すプレビューカードが WebUI に飛ぶ / [#1032](https://github.com/pooza/capsicum/issues/1032) 上へ戻るとスクロール位置が跳ねる / [#1033](https://github.com/pooza/capsicum/issues/1033) プレビューカードの幅が OGP 画像の有無で変わる。**relay は v1.61 で触っていない**（open Issue 0 件・open マイルストーン 0 件、prod の `/health` が `b9ce14c` で origin/main の HEAD と一致・2026-08-27 実測）。
