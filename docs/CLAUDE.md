...
最新リリース: **v1.65.0**（2026-09-17 タグ、build 184、pubspec 1.65.0+184、リリース PR [#1118](https://github.com/pooza/capsicum/pull/1118)、merge `e5f63574`）。**大更新なし — 読む側の実害を潰す枠**（引っ越し / 凍結の印・編集済みの印・本文カウンタ）+ ログインまわりの実機検証由来の不具合。**全 5 プラットフォーム公開完了（2026-09-18 実測）**: Android は production track の versionCode 184 が `completed`（Play API）/ **iOS・macOS とも 1.65.0 が `READY_FOR_SALE`**（ASC API・2026-09-16 公開）/ Windows は Microsoft Store が `1.65.184.0` を返す（displaycatalog）/ Linux AppImage は [GitHub Release v1.65.0](https://github.com/pooza/capsicum/releases/tag/v1.65.0)（Latest）。マイルストーン [#79](https://github.com/pooza/capsicum/milestone/79)。消化（13 件）: [#1055](https://github.com/pooza/capsicum/issues/1055) 引っ越し済み・凍結・サイレンス・削除済みの印 / [#1054](https://github.com/pooza/capsicum/issues/1054) 編集済み投稿の印 / [#1034](https://github.com/pooza/capsicum/issues/1034) 本文カウンタを Mastodon の数え方へ / [#1119](https://github.com/pooza/capsicum/issues/1119) バックアップにアカウント別設定を含める / [#1110](https://github.com/pooza/capsicum/issues/1110) 足し直しで一覧に 2 件並ぶ / [#1109](https://github.com/pooza/capsicum/issues/1109) force_login の条件付け（⚠ **#1143 で無条件へ戻した**）/ [#1112](https://github.com/pooza/capsicum/issues/1112) ログイン試行の二重起動 / [#1111](https://github.com/pooza/capsicum/issues/1111) 3 分超過の案内 / [#1136](https://github.com/pooza/capsicum/issues/1136) secure storage の関所の層抜け / [#1117](https://github.com/pooza/capsicum/issues/1117) v1.64 レビューの送り分 / [#1114](https://github.com/pooza/capsicum/issues/1114) 手順書をスキルへ / [#1142](https://github.com/pooza/capsicum/issues/1142) v2.0 開発中の 1.x リリース手順の明文化 / [#1143](https://github.com/pooza/capsicum/issues/1143) リリース前レビューでリリース前に直すもの。送り分は [#1144](https://github.com/pooza/capsicum/issues/1144)（v1.66）。**relay v1.65 は残 0 で完了**（[枠も 2026-09-18 に close](https://github.com/pooza/capsicum-relay/milestone/11)）（[relay#54](https://github.com/pooza/capsicum-relay/issues/54) WNS 接続の再利用 / [relay#64](https://github.com/pooza/capsicum-relay/issues/64) stale 再送がもう 1 本の stale を借りる）。⚠ **relay の open 8 件はすべて枠外**（#55 は #597 と同じ回 / #56 は on-hold / 有償リレー 6 件は v2.0）。

⚠⚠ **v1.65 は `develop` → `main` の現行フローが使えた最後の回。**v2.0 の実装が develop に乗ると 1.x には使えなくなるので、次からは [develop が次リリース対象でないときの出し方](#develop-が次リリース対象でないときの出し方)（`origin/main` から `release/x.y.z` を切る）になる（#1142）。

⚠⚠ **リリース前レビューの赤 5 件が、すべて「この枠で出荷する機能の中の欠陥」だった**（#1143）。#1055 が Mastodon で効いていない（`suspended` / `limited` を読んでいなかった・**JSON キーが `silenced` ではなく `limited`**）、Misskey の `movedTo` が URI ではなくユーザー ID、ワードミュートがブーストの中身に当たらない、`force_login` の判定が「接続し直す」で必ず外れる、`debugPrint` がアカウントのハンドルを breadcrumb へ載せる。⚠ **v1.64 とは逆で、既存コードではなく新規実装から出た。**

⚠⚠ **#1109 は前提ごと誤っていた。**「`force_login=true` がパスワード入力を毎回強制する」という根拠が、フォークのコードで成り立たない（参照は `can_authorize_response?` の 1 か所だけで、**同意画面を出す効果しか無い**）。加えて **capsicum はアカウント削除時にトークンを revoke しない**ので、@a を消して @b を足すと判定が false のままサーバー側の @a のトークンで `matching_token?` が成立する。**条件付けでは覆えない**と判断して無条件へ戻した。実機での再現は [#1145](https://github.com/pooza/capsicum/issues/1145)。

⚠⚠ **1 日で「手順書どおりに実行できない」が 5 件出た**（#1142 / #1143 の作業中）。stale なローカル `main` を見るゲート・`for` ループと `python3` で書かれた 16KB 整列検証・versionCode と secrets の確認手順の不在・リリース PR を ready にする step の不在。⚠ **いずれも実際に 1 回回すまで誰も気づいていなかった。**規約（インタプリタ不使用・ループ禁止）を後から足したとき、**既存の手順書がその規約に違反したまま残る**。規約を足した回に手順書を grep する運用が要る。
