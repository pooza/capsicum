...
fastlane 単体での一元管理はできないため、リリースの起点は GitHub のタグプッシュとし、タグから fastlane レーンと GitHub Actions ワークフローがそれぞれ発火する構成を想定する。デスクトップ対応の設計を固める段階で、このリリースパイプライン全体を先に描いておくこと。
