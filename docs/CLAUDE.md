...
最新リリース: **v1.60.0**（2026-08-23 タグ、build 173、pubspec 1.60.0+173、リリース PR [#1016](https://github.com/pooza/capsicum/pull/1016)、merge `fac74eab`）。**大更新なし — 1.x の集約枠・前進スロット**。公開状況（2026-08-25 実測）: **全 5 プラットフォーム公開済み** — Android 製品版（production track の versionCode 173 が `completed`）/ iOS `READY_FOR_SALE` / macOS `READY_FOR_SALE`（ASC API で実測）/ Windows は Microsoft Store の公開カタログが `9AFBB08E.capsicum_1.60.173.0_x64__8ekzzj58251a2` を返す（＝認定を通って差し替わり済み）/ **Linux AppImage + Windows MSIX** は [GitHub Release v1.60.0](https://github.com/pooza/capsicum/releases/tag/v1.60.0) で publish 済み（Latest）。マイルストーン [#74](https://github.com/pooza/capsicum/milestone/74) は close 済み。消化（8 件）: [#121](https://github.com/pooza/capsicum/issues/121) Mastodon: 投稿済みメディアの説明（ALT）編集（**この枠の看板**）/ [#975](https://github.com/pooza/capsicum/issues/975) 例外を Sentry へ流す経路の統一（生 `captureException` 30 箇所 + 複数行 `debugPrint` 16 箇所 + 静的検査）/ [#976](https://github.com/pooza/capsicum/issues/976) v1.56 レビュー緑まとめ（投稿フォームのフォント設定が保存されない 3 原因 / Mastodon エラーの日本語化ほか）/ [#1012](https://github.com/pooza/capsicum/issues/1012) v1.59 レビューの送り分 / [#1014](https://github.com/pooza/capsicum/issues/1014) v1.59 リリース PR の Codex 指摘 3 件 / [#1020](https://github.com/pooza/capsicum/issues/1020) マージ済み #1017 / #1019 に付いた Codex P1 + P2 / [#1022](https://github.com/pooza/capsicum/issues/1022) リリース前レビューの赤 4 + 黄 6 / [#1028](https://github.com/pooza/capsicum/issues/1028) 下書きスロットの save / clear / restore 競合。✅ **#121 の導線は開通済み**（2026-08-25 実測）。出荷直後はプリセット全 5 サーバーで導線が出ず、これは fail-closed が意図どおり働いている状態だった。その後モロヘイヤが **5.35.0** へ本番昇格し、Mastodon 3 台（美食丼 / デルムリン丼 / キュアスタ！）が `.config.features.media_update = true` を返している。Misskey 2 台は `false`（設計どおり）。フラグの成立条件は版番号ではなく 2 つの and で、正本は [tech-notes.md](tech-notes.md#alt-編集の導線が出る条件は版番号ではなく-2-つの-and121)。**relay は v1.60 で触っていない**（同名枠を作らないのが規約どおり。open Issue 0 件・open マイルストーン 0 件、prod の `/health` が `b9ce14c` で origin/main の HEAD と一致・2026-08-25 実測）。

**リリース前レビューは 1 巡**（5 観点・赤 4 + 黄 6 を [#1022](https://github.com/pooza/capsicum/issues/1022) で消化）。⚠ **赤 4 件は 2 系統に割れた** — (a) **Sentry への PII 流出 2 件**（`account_storage` の `delete_no_op` が #1020 で足したばかりの `accountKey` タグ規約を同じファイルの隣で破っていた / `chat_provider` の `onParseError` が `FormatException.toString()` を breadcrumb に入れており、**本文を持つ parse 経路だけが素通し**だった）、(b) **#121 が新しく立てた面 2 件**（compose の ALT に上限が無く 512 超で投稿全体が失敗 / `MaxLengthEnforcement` の既定が既存の長い ALT を 1 打鍵で黙って切る）。⚠ **(a) の 2 件目を見逃したのは検査（#975）の側に理由がある** — `breadcrumbSinks` の根が `debugPrint` 固定で `Breadcrumb` の直接組み立てを一度も見ておらず、かつ `$` 補間しか見ていなかった。**検査自身が動機に挙げていることを検査していない**形。どちらも塞いだ上で、残りの穴は [#1027](https://github.com/pooza/capsicum/issues/1027) で v1.61 へ送った。⚠ **黄にもテストの空振りが 1 件**（本文長超過の日本語化が Rails 既定形の正規表現で、Mastodon の実文面と一致しないまま**テストがコード側と同じ架空の文字列を assert**していたので緑だった）。⚠ **Codex は #1021 で 6 巡**かかり、これを機に**巡回の止め方を規約化した**（[Codex レビューの回し方](#codex-レビューの回し方open-pr)節・2026-08-23）。
