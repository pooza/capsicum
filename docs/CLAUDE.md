...
GitHub Issues のマイルストーン（v1.0 / v1.1 / v1.2 / v1.3 / v1.4 / v1.5 / v1.6 / v1.7 / v1.8 / v1.9 / v1.10 / v1.11 / v1.12 / v1.13 / v1.14 / v1.15 / v1.16 / v1.17）が正本。個別 Issue の一覧・ステータスはここに複写しない。

最新リリース: **v1.14.0**（2026-04-12、App Store / Google Play 両ストア公開済み）。v1.0.0 は 2026-03-14 にストア公開。リリース履歴の詳細は [GitHub Releases](https://github.com/pooza/capsicum/releases) を参照。
...
| プラットフォーム | 配布方式 | 費用 |
| --- | --- | --- |
| macOS | Mac App Store または `.dmg` 直配布（判断は実装着手時） | Apple Developer Program $99/年（iOS と共用、追加なし） |
| Windows | Microsoft Store を推奨（手数料 0 %、コード署名証明書を節約できる） | 個人アカウント $19 一度きり |
| Linux | Flathub（Flatpak）+ GitHub Releases で AppImage | 無料 |

- **Snap Store は候補から外す**。理由: Canonical の中央集権的な運営とオーナー自身の好み。Flatpak は似た構造だが分散的・freedesktop 寄りで別物として扱う
- **AUR・.deb / .rpm の個別パッケージは後回し**。有志メンテ任せで良い

配布ツールチェーン（fastlane + GitHub Actions のハイブリッド）:

| プラットフォーム | 主なツール |
| --- | --- |
| iOS / Android | fastlane（既存） |
| macOS (App Store / .dmg) | fastlane（`deliver` + `notarize` + `create-dmg` を組み合わせ） |
| Windows (Microsoft Store) | GitHub Actions + msstore CLI + MSIX packaging |
| Linux (Flathub) | GitHub Actions + flatpak-builder |
| Linux (AppImage) | GitHub Actions + appimagetool / linuxdeploy |

fastlane 単体での一元管理はできないため、リリースの起点は GitHub のタグプッシュとし、タグから fastlane レーンと GitHub Actions ワークフローがそれぞれ発火する構成を想定する。全体像・段階的な実装順序・シークレット管理は [release-pipeline.md](release-pipeline.md) にまとめた。

プラグインのデスクトップ対応状況は [desktop-plugin-compatibility.md](desktop-plugin-compatibility.md) に棚卸し済み。第2段階では `BackgroundTaskScheduler` / `MediaPicker` / 通知サブシステムの抽象化が主題となる。
