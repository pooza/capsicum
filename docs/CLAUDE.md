...
- `sentry-cli --auth-token <調査用トークン> issues list -p capsicum` で unresolved イベントを確認（トークンは `~/.sentryclirc` から取得: `awk '/\[auth\]/{getline; print}' ~/.sentryclirc | sed 's/token=//'`）
- 新着の unresolved イベントがあれば Sentry API（`curl -s -H "Authorization: Bearer $TOKEN" "https://sentry.io/api/0/issues/{issue_id}/"`) で詳細を取得し、対応要否を報告
- resolved 済みのイベントは報告不要
