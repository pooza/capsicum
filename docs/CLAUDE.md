...
v1.29.0（2026-05-28 タグ、お知らせ通知 C 案 [#477](https://github.com/pooza/capsicum/issues/477) を主目的とする大更新単独配置マイルストーン）。capsicum-relay 経由でプリセットサーバー (`features.announcement_push: true`) の announcement を APNs / FCM で push 配信する経路を本配線。モロヘイヤ ([pooza/mulukhiya-toot-proxy#4354](https://github.com/pooza/mulukhiya-toot-proxy/pull/4354) features.announcement_push 公開 / [#4355](https://github.com/pooza/mulukhiya-toot-proxy/pull/4355) GET /announcement/list) と capsicum-relay ([pooza/capsicum-relay#14](https://github.com/pooza/capsicum-relay/issues/14) subscription 管理 + Mastodon / Misskey polling worker、close 済み) と capsicum 側 (NotificationType.announcement / PushMessageDispatcher で復号フローから分離 / 設定 opt-out トグル / token refresh・ログアウト整合 / 通知タップで /announcements 遷移) を揃えた。デフォルト ON / 明示 OFF を記録する opt-out モデル。あわせて [#621](https://github.com/pooza/capsicum/issues/621) ログアウト時 secret_*:// 残留 (security、verify + 残骸再 delete + Sentry 報告) / [#619](https://github.com/pooza/capsicum/issues/619) 絵文字ピッカー focus / [#579](https://github.com/pooza/capsicum/issues/579) 効果側センサ撤去 を同梱。リリース前 5 観点レビュー 1 回目 (黄 3 件) はインライン消化、2 回目差分レビュー (大更新規定) は赤 0 / 黄 0、Codex P2 ([opt-out トグル表示条件](https://github.com/pooza/capsicum/pull/633#discussion_r3317178704)) は AnnouncementSubscriptionService.hasLocalState 導入 + 表示条件 OR 化 ([05fb9c6](https://github.com/pooza/capsicum/commit/05fb9c6)) で同梱解消。pubspec は当初 1.29.0+83 だが ASC 側に build 84 が既消費 (macOS v1.28.1 取り下げ再ビルド由来) で衝突したため、1.29.0+85 に bump して main 直接 commit + v1.29.0 タグを force-update した経緯あり ([bf593df](https://github.com/pooza/capsicum/commit/bf593df))。iOS App Store 審査提出済み・Android 製品版昇格済み・macOS TestFlight アップロード済み (Mac App Store 提出は pooza 手動。v1.28.0 は取り下げ済み、v1.26.0 が live で長期停滞中) ・Linux AppImage / Windows MSIX 自己署名直配 GitHub Release publish 済み・Windows Microsoft Store は pooza が Partner Center Web UI から手動 publish。

v1.28.1（2026-05-27 タグ、Android / iOS 製品版公開済み・macOS Mac App Store は v1.28.0 が取り下げられ v1.26.0 が live・Windows Microsoft Store 公開済み・Linux AppImage / Windows MSIX 自己署名直配 GitHub Release publish 済み）は v1.28.0 リリース後に把握した [PR #603](https://github.com/pooza/capsicum/pull/603) の Codex P2 と v1.28 同梱 #594「キーボードをしまう」ボタン余波の誤投稿バグをまとめたホットフィックス。`simple_post_bar` の判定軸を `_focusNode.hasFocus` から `View.of(context).viewInsets.bottom > 0` に切り替え WidgetsBindingObserver の didChangeMetrics で観測する形に変更（[#630](https://github.com/pooza/capsicum/issues/630)、@precure mk.precure.fun 報告由来）、`PagesSupport.getLikedPages` の戻り値を `List<Page>` → `List<LikedPageEntry>` (typedef `({String likeId, Page page})`) に変えてライクエントリ ID 軸の pagination cursor に修正（[#631](https://github.com/pooza/capsicum/issues/631)）、`ChatThreadListNotifier.applyRoomMessage` を追加して `ChatRoomTimelineNotifier._handleStreamMessage` から都度呼び thread list を room message でも並び替わる経路を作成（[#632](https://github.com/pooza/capsicum/issues/632)、閉じたルームは購読モデル都合で取りこぼし → 別マイルストーン送り）。縮小版リリース前レビュー (API 契約 / 並行性 2 観点) は赤 0 / 黄 5 で、閾値以下の 2 件 (#631 _refresh × _loadMoreLiked race / #632 isUnread セマンティクス) は d643bd6 でインライン消化、残り 3 件 ([#635](https://github.com/pooza/capsicum/issues/635) View.of vs MediaQuery 非対称 / [#636](https://github.com/pooza/capsicum/issues/636) autoDispose resurrect / [#637](https://github.com/pooza/capsicum/issues/637) 閉じたルーム取りこぼし) は v1.30 で扱う。iOS / macOS / Android / Linux / Windows すべて pubspec 1.28.1+82 で揃え。Sentry CAPSICUM-24 (`/api/i/page-likes` 403) は #631 と同じ Pages 動線で並走観測。release/1.28.1 を main から切って [#634](https://github.com/pooza/capsicum/pull/634) で main にマージ、その後 develop に back-merge。

**v1.28.0**（2026-05-26 タグ、Android v1.28.1 で上書き済み・iOS App Store 公開済み（v1.28.1 で再提出済み）・macOS Mac App Store ストア未反映（v1.27 系列の審査が停滞中、v1.28.1 でまとめて再提出済み・pooza 手動）・Linux AppImage / Windows MSIX 自己署名直配 GitHub Release publish 済み・Windows Microsoft Store v1.28.1 で再提出済み（pooza が Partner Center Web UI から手動 publish））は Misskey chat ルーム機能（グループチャット、[#438](https://github.com/pooza/capsicum/issues/438)）を主目的とする大更新単独配置マイルストーン。同居で Misskey Pages 読み取り対応（静的ブロック限定 v1 ビューア、[#186](https://github.com/pooza/capsicum/issues/186)）と Misskey ドライブの複数選択一括移動（[#567](https://github.com/pooza/capsicum/issues/567)）も取り込んだ。あわせて #594「キーボードをしまう」ボタン、#609 Mastodon 投稿フォームの未登録 shortcode 警告、#606 features.media_catalog ゲート、#602 streaming 観測の throttle 分離 + reconnect 上限 UI、#601 timeline ページ取得ループ上限、#600 Mastodon streaming 経由投稿の adminRoleIds 伝播修正、#620 Linux OAuth 既存クライアント救済を消化。リリース前 5 観点レビューを 1 回目 (commits d4a8526〜e8f00d7 の 7 件追従) + 大更新につき 2 回目差分レビューも実施し、黄 3 件 (#601 cap breadcrumb / #186 retry guard / #567 mounted ガード) は閾値以下のため 95df769 でインライン消化、赤 0 件。iOS / macOS は build 81 起点 (v1.27.1 で 80 を消費済み)、Linux / Windows / Android も pubspec 1.28.0+81 で揃えた。
...
3. **第3段階: Linux 対応（v1.24、Linux 配布パイプライン整備済み。Windows は v1.25 で MSIX 自己署名直配を再開、v1.27 で Microsoft Store 公開達成）** — 第2段階で通知周りが整理され、プラグイン依存の棚卸しが済んでから本格着手。Linux の配布形態は当初 Flathub + AppImage を計画（[#424](https://github.com/pooza/capsicum/issues/424)）したが、**Flathub は 2026-05-29 に断念**（提出 PR [flathub/flathub#8833](https://github.com/flathub/flathub/pull/8833) が Flathub 管理者により AI Slop 判定。capsicum が実質 AI 製で Generative AI policy と構造的に相性が悪く、再提出は永久 ban リスク。[#604](https://github.com/pooza/capsicum/issues/604) / [#470](https://github.com/pooza/capsicum/issues/470) close）。**現状 Linux 配布は AppImage 単独**（v1.24 以降 GitHub Releases から即時配布、手順は [store-release-guide.md §4.5](store-release-guide.md)）。後継施策として AppImage のデスクトップ統合 install script ([#640](https://github.com/pooza/capsicum/issues/640)) と直配チャネル更新検知 ([#641](https://github.com/pooza/capsicum/issues/641)) を Issue 化。Linux 実機検証は [#425](https://github.com/pooza/capsicum/issues/425)、macOS 実機検証は [#494](https://github.com/pooza/capsicum/issues/494) で並走。Windows は **2026-05-12 に v1.25 で再開と確定** — v1.25 時点では Microsoft Store 公開は msstore CLI 自動 publish 経路の詰まり (個人開発者アカウントから Entra ID テナント関連付け UI に到達不可) で保留、AppImage と同様に **GitHub Releases 経由の自己署名 MSIX 直配** で短期に Windows 配布を再開する方針 ([#423](https://github.com/pooza/capsicum/issues/423) / [#483](https://github.com/pooza/capsicum/issues/483))。`feature/423-windows-distribution` ブランチに Phase 1〜4 (Windows scaffold / `msix_config` / `windows-release.yml`) + Phase 5 msstore CLI publish step (secrets 未投入時 skip) を実装済み、develop に再合流して self-signed cert 生成 + MSIX 署名 + GitHub Releases 添付を CI に追加する。配布対象は「証明書 import を厭わない上級ユーザー」と明示。[#474](https://github.com/pooza/capsicum/issues/474) push 本配線・[#484](https://github.com/pooza/capsicum/issues/484) SMTC NowPlaying は ship 必須でないため引き続き on-hold。Microsoft Store 公開は **2026-05-15 に再評価**: msstore CLI 経路の詰まりは「Web UI 手動 publish ルートは Entra ID 関連付け不要」という別経路で迂回可能と判断、[#544](https://github.com/pooza/capsicum/issues/544) を Web UI 手動 publish ルートで v1.27 再開、**2026-05-20 に初回審査通過で Store 公開達成** ([apps.microsoft.com/detail/9np2gr7m2w6p](https://apps.microsoft.com/detail/9np2gr7m2w6p))。msstore CLI 自動 publish は引き続き保留（毎リリース Partner Center Web UI から手動 publish、手順は [store-release-guide.md §4.6](store-release-guide.md) 参照）、自己署名直配 ([#423](https://github.com/pooza/capsicum/issues/423)) は Store 認定中の先行配布・上級者向け補助路線として継続。中期施策の **コード署名証明書取得** ([#534](https://github.com/pooza/capsicum/issues/534)) は Store 経由配布が主ルートになったため当面不要（Store 側で再署名されるため `msix_config.store: false` の self-signed MSIX のまま submit 可）。video_player → media_kit の本移行は v1.21 の TestFlight Internal 検証で video_player が macOS 上で再生・添付・投稿とも問題なく動作することが確認できた（pooza が動画つき投稿で意図的に検証）ため緊急性が下がっており、第3段階の必須スコープからは外す。ただし **Linux は video_player に Linux 実装がなく動画再生不可**（2026-05-10 Debian 13 + AppImage v1.24.0 で「動画を読み込めません」を確認。クラッシュはせず graceful fallback。Windows も video_player_win 棚上げで不可）。macOS は video_player で動作（v1.21 検証済み）。Linux / Windows の動画再生有効化は media_kit 移行 [#492](https://github.com/pooza/capsicum/issues/492) で対応し、v1.30 にアサイン（**develop で実装完了。video_player を撤去し media_kit 一本化。Linux はビルド時 libmpv-dev + 実行時 libmpv.so.2 を linuxdeploy が AppImage 同梱**）。v1.23 リリース前レビューで挙がった desktop 補修系（macOS Keychain Access Group 明示・FileSelectorMediaPicker の videoGroup 拡張・TimerBackgroundTaskScheduler 堅牢化等）も v1.24 に同居

第1段階と第2段階のあいだに **v1.22: Misskey メッセージ機能対応** を単独配置した（大更新を他の大更新と並走させない方針）。v1.22 として 2026-05-01 にリリース完了。リリース前レビューの黄判定や追加報告から派生したバグ修正・enhancement (#442 系列、#449、#440 など) は v1.22.x ホットフィックスではなく **v1.25**（Misskey メッセージ改善マイルストーン）にまとめて消化する方針 — Misskey メッセージは現状実験的機能の位置付けで、ホットフィックス級の利用状況・致命度には達していないため。グループチャット (#438) は単独機能として **v1.28** に分離。v1.28 は Misskey テーマに一本化し、#438（大更新）+ **Pages 機能の読み取り対応** ([#186](https://github.com/pooza/capsicum/issues/186)、静的ブロック限定 v1、AiScript なし) + ドライブ複数選択一括移動 ([#567](https://github.com/pooza/capsicum/issues/567)) の 3 件構成（2026-05-16 にマイルストーン現実化で確定）。v1.29 は **お知らせ通知 C 案** ([#477](https://github.com/pooza/capsicum/issues/477)、モロヘイヤ + 自前 SNS フォーク + capsicum-relay 経由でプリセットサーバー向け push 配信) を主目的に単独配置（2026-05-13 再開、大更新）。あわせて低工数の小粒 [#619](https://github.com/pooza/capsicum/issues/619) 絵文字ピッカーのフォーカス / [#621](https://github.com/pooza/capsicum/issues/621) ログアウト時 secret_*:// 残留 (security) を同居。当初は [#604](https://github.com/pooza/capsicum/issues/604) Flathub 再提出も同居予定だったが、2026-05-29 に提出 PR が AI Slop 判定で block、Flathub 対応自体を断念して #604 close。v1.30 は **video_player → media_kit 移行 ([#492](https://github.com/pooza/capsicum/issues/492)) を主役に格上げ**（2026-05-29 再構成。当初は集約枠だったが、Flathub 断念で Linux 配布が AppImage 単独に確定し Linux 動画再生不可の解消優先度が上がったため）。あわせて Flathub スコープアウトの代償として Linux AppImage 配布の体験補強を巻き上げ取り込み ([#589](https://github.com/pooza/capsicum/issues/589) hicolor icon / [#640](https://github.com/pooza/capsicum/issues/640) install script / [#641](https://github.com/pooza/capsicum/issues/641) 直配チャネル更新検知)。desktop UX 中粒 (#574 / #571 / #572 / #575) と v1.28 残務小〜中粒 (#637 / #636 / #635 / #629 / #626 / #624 / #623 / #622)、#642 macOS OAuth assertion (Sentry CAPSICUM-1R)、#605 サポーターバッジプロフィール、#523 push_failure_recorder enum 化、#577 (on-hold) で構成。**v1.32**（2026-05-16 新設）は desktop / packaging 系リファクタ overflow を集約（大更新なし）。**v1.34**（2026-05-20 新設）は当初集約枠だったが、2026-05-29 再構成で **desktop 通知本配線テーマに格上げ** — Linux push 方針 (#475) で確定した「desktop 3 OS WebSocket streaming → OS ローカル通知」設計 ([desktop-notification-design.md](desktop-notification-design.md)) の本配線 ([#569](https://github.com/pooza/capsicum/issues/569) 主役、大更新) + macOS push 本配線 ([#468](https://github.com/pooza/capsicum/issues/468)、APNs 直接 / capsicum-relay 拡張) を同テーマで同居。Annict 連携の小粒 (#611 / #593 / #592) + #576 投稿フォーム文字オーバーレイ も同居。**v1.36**（2026-05-29 新設）は集約枠で v1.28 出荷機能の Misskey chat / Pages 続編 (#612 chat リアクション / #613 chat ファイル添付 / #615 Pages like・unlike / #617 Pages 『人気』セクション) + #428 投げ銭の横展開 (#598 macOS IAP / #599 Windows IAP / #596 サーバー側保持 = B-4) を集約。これら集約枠は個別 Issue 構成を [GitHub Milestones](https://github.com/pooza/capsicum/milestones) を正本とし、ここには列挙しない。**v1.35**（2026-05-24 新設）は **投稿フォームに劇中ワード / ハッシュタグ / カスタム絵文字のサジェスト機能** ([#614](https://github.com/pooza/capsicum/issues/614)) を単独配置する大更新マイルストーン。投稿フォーム新規 UI レイヤ + モロヘイヤ tagging API（`POST /mulukhiya/api/tagging/tag/search`）/ カスタム絵文字 API 統合が主題のため他の大更新と並走させない。実況時に OS IME に登録がない専門ワード（例: `閃華裂光拳`）を一発で挿入したいユーザー要望が起点。**v1.33**（2026-05-16 新設）は **NowPlaying 横断対応**（#466 NowPlayingProvider 抽象 + Linux MPRIS / #484 Windows SMTC / #570 Spotify ナウプレ + mulukhiya OAuth）— #570 が大更新級のため実質単独配置扱い。既存ナウプレが OS 依存で Linux/Windows 不可な点の横断的代替が目的（#466 は当初 v1.28 合流予定だったが、#438 大更新との二大テーマ並走回避のため分離）。v1.31 は **タイムライン上の各投稿へのタッチ操作対応** ([#565](https://github.com/pooza/capsicum/issues/565)) を単独配置で新設（2026-05-14、大更新）— 誤タッチ防止のため意図的に塞いでいた WebUI 風のタッチ操作を、端末ごと・操作別にオプトインで有効化する。久々に「触り心地そのもの」に効く更新のため大更新として丁寧に扱う。**v1.31.0 として 2026-06-03 リリース完了**（Android 製品版 / Windows Microsoft Store 公開済み（2026-06-03 審査通過）/ Linux・Windows GitHub Release publish 済み / iOS・macOS は審査提出済みでストアは v1.30.0 が live（審査待ち）。リリース前レビューで #643 移行のシークレット保全・#649 breadcrumb スクラブ・#654 コメントを追加修正）。リリース直後の Sentry 観測で v1.31.0 のアクションメニュー（翻訳項目の `Localizations.localeOf(context)` が外側 PostTile context を参照し BottomSheet rebuild 時に null check fatal）の新規 fatal を検出し [#659](https://github.com/pooza/capsicum/issues/659)（CAPSICUM-2G、ホットフィックス候補）として起票。**v1.31.1 として 2026-06-04 にホットフィックスリリース完了**（#659 単独スコープ。メニューを開いた時点で locale を一度だけ解決して閉包に取り込み、`messenger` 等と同じ resolve-before-show パターンで volatile な外側 context への再依存を除去。縮小版リリース前レビュー（並行性・ライフサイクル観点）赤0/黄0。`release/1.31.1` を main から分岐し直接 commit（cherry-pick なし、[e3f2368](https://github.com/pooza/capsicum/commit/e3f2368)）、[#663](https://github.com/pooza/capsicum/pull/663) でマージ後 develop に back-merge。pubspec 1.31.1+94。Android 製品版昇格済み / iOS App Store 審査提出済み / macOS TestFlight アップロード済み・Mac App Store 審査提出済み（pooza 手動）/ Linux AppImage・Windows MSIX を [GitHub Release v1.31.1](https://github.com/pooza/capsicum/releases/tag/v1.31.1) で配布・Windows Microsoft Store も審査提出済み（pooza が Partner Center Web UI から手動）。全プラットフォーム提出完了）。
