...
GitHub Issues のマイルストーン（v1.0 / v1.1 / v1.2 / v1.3 / v1.4 / v1.5 / v1.6 / v1.7）が正本。個別 Issue の一覧・ステータスはここに複写しない。
...
- **v1.5** — ユーザー要望の消化（簡易投稿バー・rel=me バッジ・複数 URL プレビュー・Misskey クリップ/絵文字パレット・メディアタブ 等）
- **v1.6** — ユーザー要望の継続消化（翻訳・ハッシュタグフォロー/デッキ固定・Misskey Play/ギャラリー・予約投稿タグ編集 等）
- **v1.7** — 追加機能（通知統合一覧・実況タグ切替・Misskey ドライブ・メディアカタログ 等）
