...
- **2026-09-12 追記**: 未決事項 **2（カラムの購読モデル）**と **4（モバイルで出すか → 出す）を決着**し、**決定済み事項 4〜7**（カラムキーの形 / カラムと購読の生死 / カラム列の編集 / ナビゲーションと AppBar）を追加した。⚠ **決定の要は「接続方式を今決めない」**——`StreamSupport` をキー付きにすると、1 ソケット多重化かカラムごとソケットかは実装の内部事情に落ちる。⚠ **残る未決は 3（#887 の保証）と 5（バックアップ互換）の 2 つで、どちらもフェーズ 3**
