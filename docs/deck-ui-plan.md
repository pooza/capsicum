...
### 8. ⚠ capsicum の投稿タイルが読める最小幅（実測が要る）

**未決事項 1 から残った唯一の問い。**SubwayTooter は 300dp を既定にしているが、capsicum のタイルで成立するかは測っていない。

- 測り方: 既存の `PostTile` を幅可変のコンテナに入れ、**実機で**幅を落としていって破綻点を見る。⚠ **デスクトップ GUI は Claude から観測できない**（`reference_desktop_gui_not_observable`）ので、**この確認は pooza の手が要る**
- ⚠ **「600px」は最小幅ではなく、ドロワー閾値を決めるための想定値**だった（`home_screen.dart:574` のコメント）。**capsicum は最小可読幅を一度も確定させていない**
- 見るもの: 本文の折り返し / 横長カスタム絵文字（[#858](https://github.com/pooza/capsicum/issues/858)）/ プレビューカード / アクション行 / 添付画像のサムネイル
- **カラム幅をユーザー設定にする**（SubwayTooter と同じ）なら、確定させるのは**下限値だけ**でよい
