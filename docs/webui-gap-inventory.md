...
**この文書の時点では Issue を起こさない**（#993 と同じ順序。分類が確定してから起票する）。

起票を提案する順序:

1. **A-2 フォロー中のハッシュタグ一覧**（[#1039](https://github.com/pooza/capsicum/issues/1039) と構造が同一で、あちらが v1.63 の主役。**設計を流用できるので一緒にやると安い**）
2. **A-1 お気に入り一覧**（ブックマーク画面が既にあるので、その隣に 1 枚足す形。小粒）
3. **A-3 引用一覧**（`_showRebloggedBy` と同じ BottomSheet の形。小粒）
4. B-3 フィーチャータグ（読む側だけ先行なら中粒）
5. B-1 Pages の作成・編集（#1050〜#1053 と束ねる判断もありうる）
6. B-2 Play の作成・編集（AiScript エディタ。**単独で判断**）

### ⚠ 行き先は未決（pooza の判断待ち）

**A 群を [v1.63](https://github.com/pooza/capsicum/milestone/77) に入れてよいかは判断が要る。**v1.63 は「**#993 の棚卸し成果を消化する専用枠**」として作られ、description に「**この枠は内部由来を入れてよい枠ではない**」と明記してある。A 群は #991 の成果なので、

- **同じ「棚卸しの成果」だから v1.63 で消化する**（枠の趣旨を「棚卸し由来」と読む）
- **#993 専用の枠なので混ぜず、v1.64 か新しい枠へ**（枠の趣旨を「#993 の成果」と読む）

のどちらとも読める。⚠ **#993 のときも同じ論点で 1 回止めて pooza が決めている**（[api-gap-inventory.md](api-gap-inventory.md) §8「起票の行き先」）ので、ここも決めてから起票する。
